...
void PipelineBuilder::destroyPipeline(VkDevice device, Pipeline pipeline) {
...
VkPipeline PipelineBuilder::buildGraphicsPipeline(VkDevice device) {
...
VkPipeline PipelineBuilder::buildComputePipeline(VkDevice device) {
    ...
    VkPipeline newPipeline;
    VK_CHECK(
        vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &newPipeline));
    return newPipeline;
}
